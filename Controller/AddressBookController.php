...
class AddressBookController extends Controller
{
     ...
     *      name="orocrm_dotmailer_synchronize_adddress_book_datafields",
     *      requirements={"id"="\d+"}
     * )
     * @Acl(
     *      id="orocrm_dotmailer_address_book_update",
    ...
    {
        try {
            $this->getDoctrine()->getRepository('OroCRMDotmailerBundle:AddressBookContact')
                ->bulkEntityUpdatedByAddressBook($addressBook);

            $status = Codes::HTTP_OK;
            $response = [
                'message' => $this->get('translator')->trans('orocrm.dotmailer.addressbook.sync_datafields_success')
            ];
        } catch (\Exception $e) {
            $status = Codes::HTTP_BAD_REQUEST;
            $response['message'] = sprintf(
                $this->get('translator')->trans('orocrm.dotmailer.addressbook.sync_datafields_failed'),
                $e->getMessage()
            );
        }
     ...
     * @Route(
     *      "/create",
     *      name="orocrm_dotmailer_address_book_create"
     * )
     * @Acl(
     *      id="orocrm_dotmailer_address_book_create",
     *      type="entity",
     *      permission="CREATE",
     *      class="OroCRMDotmailerBundle:AddressBook"
     * )
     * @Template("OroCRMDotmailerBundle:AddressBook:update.html.twig")
     *
     * @return array
     */
    ...
    protected function update(AddressBook $addressBook)
    {
        return $this->get('oro_form.model.update_handler')->update(
            $addressBook,
            $this->get('orocrm_dotmailer.form.address_book'),
            $this->get('translator')->trans('orocrm.dotmailer.addressbook.message.saved'),
            $this->get('orocrm_dotmailer.form.handler.address_book_update')
        );
    }
}
